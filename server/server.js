...
[00:18] Langsung checkout di keranjang pojok kiri bawah sekarang juga!`;
      ...
      scriptData = {
        ...
        caption: `Rekomendasi terbaik! ${productTitle || 'Produk viral'} praktis dan berkualitas. Langsung checkout ya! #racunshopee #shopeehaul #viral #rekomendasi`,
        lexicon_to_replace: {},
      };
    }
...
function syncVideoToAndroidStorage(finalOutputPath, finalFileName, projectName = 'clipper') {
...
app.post('/api/regenerate-voiceover', async (req, res) => {
  reloadEnvironment();
  const { jobId, customScript, lexicon } = req.body;

  if (!jobId) return res.status(400).json({ error: 'Job ID is required.' });

  try {
    const updatedJob = await processJobVoiceover(jobId, customScript, { lexicon });
...
app.post('/api/retry-job-tts', async (req, res) => {
  reloadEnvironment();
  const { jobId, customScript, apiKey, aiProvider } = req.body;
